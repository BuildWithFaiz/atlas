...
class ClerkJWTMiddleware:
    ...
    def __call__(self, request):
            ...
            if request.path.startswith('/api/search/'):
                ...
                if auth_header.startswith('Bearer '):
                    ...
                    if user_info:
                        ...
                        if not self.clerk_secret_key:
                            request.clerk_user_id = 'dev_user_default'
                            request.clerk_user = {'sub': 'dev_user_default'}
        ...
        if not auth_header.startswith('Bearer '):
            if self._requires_auth(request.path):
                # In development without Clerk, allow with fallback user
                if not self.clerk_secret_key:
                    logger.info(f"Development mode: Using fallback user for {request.path}")
                    request.clerk_user_id = 'dev_user_default'
                    request.clerk_user = {'sub': 'dev_user_default', 'email': 'dev@example.com'}
                    return self.get_response(request)
        ...
        try:
            ...
            if self._requires_auth(request.path):
                # In development without Clerk, allow with fallback user
                if not self.clerk_secret_key:
                    logger.info(f"Development mode: Using fallback user after error for {request.path}")
                    request.clerk_user_id = 'dev_user_default'
                    request.clerk_user = {'sub': 'dev_user_default', 'email': 'dev@example.com'}
                    return self.get_response(request)
    ...
    def _verify_token(self, token: str) -> dict | None:
        if not self.clerk_secret_key:
            ...
            # In development mode, use a single default user ID for all requests
            # This ensures all documents uploaded in dev mode are visible to all requests
            # If you need multi-user testing, set CLERK_SECRET_KEY
            logger.info("Fallback auth: Using default dev user for all requests")
            return {
        ...
        try:
            # Try RS256 first (Clerk's default algorithm)
            ...
            # Fallback: try with both algorithms allowed
            try:
                decoded_token = jwt.decode(
                    token,
                    self.clerk_secret_key,
