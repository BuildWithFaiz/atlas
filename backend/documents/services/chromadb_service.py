...
class ChromaDBService:
    ...
    def add_document_chunks(self, document_id: str, chunks: List[Dict], clerk_user_id: str = None, batch_size: int = 100) -> bool:
        """
        Add document chunks to ChromaDB in batches to handle large PDFs efficiently.
        
        Args:
            document_id: The document ID
            chunks: List of chunk dictionaries
            clerk_user_id: Optional user ID for filtering
        ...
        try:
            ...
            logger.info(f"Adding {total_chunks} chunks for document {document_id} in batches of {batch_size}")
            
            # Process chunks in batches to avoid memory issues and ChromaDB limits
            ...
            for batch_start in range(0, total_chunks, batch_size):
                ...
                try:
                    ...
                    for chunk in batch_chunks:
                        ...
                        # Add clerk_user_id to metadata if provided
                        if clerk_user_id:
                    ...
                    logger.debug(f"Added batch {batch_start//batch_size + 1} ({len(batch_chunks)} chunks) for document {document_id}")
                    
                except Exception as e:
                    failed_batches += 1
                    logger.error(f"Error adding batch {batch_start//batch_size + 1} for document {document_id}: {str(e)}")
                    # Continue with next batch instead of failing completely
                    continue
            
            if failed_batches > 0:
                logger.warning(f"Completed adding chunks for document {document_id}: {successful_batches} batches succeeded, {failed_batches} batches failed")
                # Return False if all batches failed, True if at least some succeeded
                return successful_batches > 0
            
            logger.info(f"Successfully added all {total_chunks} chunks for document {document_id} in {successful_batches} batches")
            return True
            
        except Exception as e:
            logger.error(f"Error adding chunks for document {document_id}: {str(e)}", exc_info=True)
            return False
    
    def search_similar_chunks(self, query: str, n_results: int = 10, 
                            document_ids: Optional[List[str]] = None,
                            clerk_user_id: Optional[str] = None) -> List[Dict]:
        try:
            ...
            if clerk_user_id:
                conditions.append({"clerk_user_id": clerk_user_id})
    ...
    def delete_document_chunks(self, document_id: str, clerk_user_id: Optional[str] = None) -> bool:
        try:
    ...
    def hybrid_search(self, query: str, n_results: int = 10, top_k: int = 50, epsilon: float = 0.02, include_tfidf: bool = True, document_ids: Optional[List[str]] = None, unique_citations: bool = True, clerk_user_id: Optional[str] = None) -> List[Dict]:
        try:
            where_clause = None
            conditions = []
